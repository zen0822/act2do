...
class List extends Component {
    constructor(props) {
        ...
        let pageData = this._initPageData(props.page)
        let listItem = this._initListData({
            listItem: props.initItem,
            pageData,
            replace: true
        })
        let listItemEmpty = listItem.length === 0
    ...
    _initPageData(pageData) {
        if (this.props.auto) {
            let listLength = this.props.initItem.length
    ...
    async update(dataHandler) {
        return new Promise((resolve, reject) => {
            this.setState({
                ...
                try {
                    let response = typeof dataHandler === 'function'
                        ? await dataHandler()
                        : dataHandler
                    let listItem = this._initListData({
                    ...
                    let pageData = this._initPageData(response.page)
