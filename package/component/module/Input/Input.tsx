 ...
 * @prop {regexp} regex - 验证值的正则(需要注意转义的字符)
 ...
 * @prop {string} verifiedType - 验证值的类型  'number' | 'url' | 'phone' | 'tel' | 'email' | 'password'
 *
...
const Input: RefForwardingComponent<Api, InputPropTypes> = ({
  ...
  const intl = useIntl()
  let regexObj: RegExp | null = null // 正则校验的正则表达式
  ...
  function verify(opt: TVerifyOpt = {}): boolean {
    const {
      verifyRegex = true,
      firstVerify = false
    } = opt

    let { value = stateValue } = opt
    ...
    if (!value && value !== 0) {
      ...
      if (verifyRegex && (regex || verifiedType) && regexObj && !regexObj.test(String(value))) {
        verified = false
  ...
  function onBlurHandler(event: FocusEvent<HTMLInputElement | HTMLTextAreaElement>): void {
  ...
  function onKeyPressHandler(event: KeyboardEvent): void {
  ...
  useEffect(() => {
    if (!stateVerified) {
      document.body.scrollTop = (clientOffset(refMe.current) as any).top
