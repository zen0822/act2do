 ...
 * @prop {number | string} boxHeight - 列表内容高度
...
const List: FC<TypeItemProps> = ({
  ...
  const [itemLoadingDisplay, setItemLoadingDisplay] = useState(false)
  ...
  const pageTypeMore = useMemo(() => (pageType === 'more'), [pageType])
  ...
  function getPageData(pageData: TInitPage): TPage {
    if (auto) {
      ...
      return {
        length: listLength,
        size,
        total: Math.ceil(listLength / size),
        current: pageData.current + 1
      }
    } else {
      const pageSizeTmp = pageData.size || pageSize
      const pageLenght = pageData.length || (pageData.total * pageSizeTmp)
  ...
  /**
   * pageType = more 时的分页函数
   */
  async function morePageOnSwitch(): Promise<void | boolean> {
    if (statePage.total === statePage.current || !pageDisplay) {
      return false
    }
   ...
   * pageType = num 时的分页函数
   */
  async function numPageOnSwitch(pageNum: number): Promise<void | boolean> {
    if (!onSwitch) {
      return false
    }

    setItemLoadingDisplay(true)
    ...
    try {
      ...
      const [listItem, page] = await onSwitch(pageData)
   ...
   * 切换分页时
   */
  function onSwitchHandler(pageNum?: number): any {
    if (pageNum) {
      return numPageOnSwitch(pageNum)
    } else {
      return morePageOnSwitch()
    }
  }
  ...
  function initListItem(): void {
    ...
    setStatePage(pageData)
    setStateItem(listItem)
    setPageDisplay(pageData.total !== pageData.current)
  ...
  const contentEle = itemLoadingDisplay
    ? (
      <Row justify='justify'>
  ...
  if (auto) {
    ...
    Object.keys(initPage).every((key) => {
      if (initPage[key as TPageEle] !== preInitPage[key as TPageEle]) {
        ...
        setPageDisplay(pageData.current !== pageData.total)
        setStatePage(pageData)
  ...
  return (
    ...
    >
      ...
      {stateItemEmpty
        ? (
          ...
          <div className={_xclass('box')} ref={refBox}>
            <div
              style={{
                minHeight: props.boxHeight || undefined
              }}
            >{contentEle}</div>
      ...
      {pager
        && pageTypeMore
        && noMoreDataHint
        && statePage.current === statePage.total
        && statePage.total > 1
        && (
